...
build-all: ## Build application for supported architectures
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=1 GODEBUG=netdns=cgo go build -ldflags="-X 'main.Version=${version}'" -o build/${BINARY_NAME}-linux-x86_64 checker.go
	...
	GOOS=linux GOARCH=arm64 CGO_ENABLED=1 CC=aarch64-linux-gnu-gcc go build -ldflags="-X 'main.Version=${version}'" -o build/${BINARY_NAME}-linux-aarch64 checker.go
